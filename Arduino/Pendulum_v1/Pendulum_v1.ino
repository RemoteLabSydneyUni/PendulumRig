...
void updatePeriod(){
  if (periodCounter >= 2){
    period = (float)(millis()-periodTimer);
    periodTimer = millis();
    periodCounter = 0;
  }
}
...
void loop() {
  if ((digitalRead(PERIOD_SCAN)== HIGH)&&(previousScan==false)){
    periodCounter = periodCounter +1;
    previousScan = true;
    testTimer = millis();
  }else if ((digitalRead(PERIOD_SCAN)== LOW)&&(previousScan==true)){
    previousScan = false;
    testTime = millis()-testTimer;
  }
